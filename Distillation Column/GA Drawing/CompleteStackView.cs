    ...
    class CompleteStackView : CDrawingView
    {
        ...
        public override void Generate(string jsonStr)
        {
            try
            {
                ...
                List<JToken> platformTokenList = jsonData1["Platform"].ToList();
                foreach (JToken item in platformTokenList.Reverse<JToken>())
                {
                    platFromElevation = (float)item["Elevation"] + _global.Origin.Z;
                ...
                //{
                //    double elevation = (double)(item["elevation"])  + _global;
